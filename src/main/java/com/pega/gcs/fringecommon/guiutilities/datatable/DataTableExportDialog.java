...
public class DataTableExportDialog extends JDialog {

    private static final long serialVersionUID = 1563645482258326358L;

    private static final Log4j2Helper LOG = new Log4j2Helper(DataTableExportDialog.class);
    ...
    private DataTableExportTask dataTableExportTask;

    private File exportFile;

    public DataTableExportDialog(AbstractDataTableModel<?, ?> abstractDataTableModel, String context, ImageIcon appIcon,
            Component parent) {
        ...
        this.dataTableExportTask = null;

        String filePath = abstractDataTableModel.getFilePath();

        exportFile = new File(filePath);

        // check if the default output file is available
        String fileName = getDefaultTSVFileName();
        File currentDirectory = exportFile.getParentFile();
        ...
        setTitle("Export to TSV - " + abstractDataTableModel.getModelName());
    ...
    private void performPALExport() {
        ...
        if (returnValue == JFileChooser.APPROVE_OPTION) {
            ...
            if (returnValue == JOptionPane.YES_OPTION) {
                ...
                final int rowCount = abstractDataTableModel.getRowCount();

                dataTableExportTask = new DataTableExportTask(abstractDataTableModel, exportFile) {
    ...
    private void cancelTask() {
        ...
        if ((dataTableExportTask != null)
                && ((!dataTableExportTask.isCancelled()) || (!dataTableExportTask.isDone()))) {
            dataTableExportTask.cancel(true);
        }
    }
}
